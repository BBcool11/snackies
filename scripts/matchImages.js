...
async function processImages() {
  ...
  const files = fs.readdirSync(imagesDir)
    ...
    .filter(f => f.startsWith('snack-')); // 只处理 snack-xxx.png 格式
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    console.log(`\n  OCR: ${text.substring(0, 100).replace(/\n/g, ' ')}...`);
    ...
    if (match) {
      ...
      unmatchedImages.push({ id: imageId, text: text.substring(0, 200) });
...
// 运行
console.log('Starting image matching...\n');
